...
def create_workflow() -> Workflow:
    ...
    """
    upper_case = UpperCase(executor_id="upper_case_executor")

    return WorkflowBuilder(start_executor=upper_case).add_edge(upper_case, reverse_text).build()


def configure_visualization_environment() -> Path:
    """Load optional visualization settings from environment variables."""
    graphviz_bin = os.getenv("GRAPHVIZ_BIN")
    if graphviz_bin:
        current_path = os.environ.get("PATH", "")
        path_entries = current_path.split(os.pathsep) if current_path else []
        if graphviz_bin not in path_entries:
            os.environ["PATH"] = f"{graphviz_bin}{os.pathsep}{current_path}" if current_path else graphviz_bin

    output_dir = Path(os.getenv("WORKFLOW_VIZ_OUTPUT_DIR", "docs"))
    output_dir.mkdir(exist_ok=True)
    return output_dir


def print_workflow_visualization(name: str, workflow: Workflow) -> None:
    """Print and save workflow visualizations."""
    print(f"\n{name} visualization:")
    print(WorkflowViz(workflow).to_mermaid())

    docs_dir = configure_visualization_environment()
    svg_path = docs_dir / f"{name.lower().replace(' ', '_')}_architecture.svg"

    try:
        saved_path = WorkflowViz(workflow).save_svg(str(svg_path))
        print(f"SVG saved to: {saved_path}")
...
async def main():
    ...
    print_workflow_visualization("Workflow 1", workflow1)
    ...
    # -------------------------------------------------------
    upper_case = UpperCase(executor_id="upper_case_executor")
    exclamation_adder = ExclamationAdder(executor_id="exclamation_adder")
    ...
    print_workflow_visualization("Workflow 2", workflow2)
